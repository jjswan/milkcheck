...
const Btn = styled.button`
  ...
  `;
function EatToEaten({text, id, category}:InamesAtom){
  ...
  const onClick=(event:React.MouseEvent<HTMLButtonElement>)=>{
    setNames(names=>{
      ...
      const eatenname = {text, id, category:"EATEN" as any};
      return [...names.slice(0, targetIndex), eatenname, ...names.slice(targetIndex+1)]
    });
  }
  return (
    <span>
      {category ==="EAT" && <Btn name="EAT" onClick={onClick}>{text}</Btn>}
    </span>
  )
}

export default EatToEaten;